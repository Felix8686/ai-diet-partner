...
import { loadFoodEnvironment, loadProfile, saveProfile, saveShoppingList, saveWeeklyPlan } from "@/lib/storage";
...
const initialProfile: OnboardingProfile = {
  age: "", sex: "", heightCm: "", weightKg: "", goals: ["减脂"], weeklyFoodBudget: "",
  weekdayCookTime: "15–30 分钟", weekdayWeekendDifference: "工作日更忙，周末有时间", outsideMealRatio: "每周约 4–7 顿",
  mealScenes: ["公司食堂"], likedFoods: "", dislikedFoods: "", dietaryRestrictions: "", breakfastPattern: "大多数时候会吃",
  lateNightSnack: "偶尔", snackHabit: "偶尔", kitchenCapabilities: ["炒", "煮", "微波"], shoppingPlace: "",
};
...
export default function OnboardingPage() {
  ...
  useEffect(() => { const savedProfile = loadProfile(); if (savedProfile) setProfile(savedProfile); }, []);
  ...
  function toggleGoal(goal: Goal) {
    setGoalError("");
    setProfile((current) => {
      const currentGoals = current.goals ?? (current.goal ? [current.goal] : []);
      const nextGoals = currentGoals.includes(goal) ? currentGoals.filter((item) => item !== goal) : [...currentGoals, goal];
      return { ...current, goals: nextGoals, goal: undefined };
    });
  }

  function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (step === 0 && (profile.goals?.length ?? 0) < 1) {
      setGoalError("至少选择一个方向。");
      return;
    }
    if (step < 2) { setStep((current) => current + 1); return; }
    const weekStart = getLocalWeekStartKey();
    const plan = generateWeekPlan(profile, weekStart, undefined, loadFoodEnvironment());
    saveProfile(profile);
    saveWeeklyPlan(plan);
  ...
  const selectedGoals = profile.goals ?? (profile.goal ? [profile.goal] : []);

  return (
    <main className="appShell onboardingShell">
      <header className="pageHeader centered"><p className="homeKicker">第一次使用</p><h1>先了解一下你的日常</h1><p>用 3 步告诉我，什么方案对你更现实。</p></header>
      <div className="stepper" aria-label={`第 ${step + 1} 步，共 3 步`}>{[0, 1, 2].map((item) => <span key={item} className={item <= step ? "step active" : "step"} />)}</div>
      <div className="onboardingTitle"><p>{step + 1}/3</p><h2>{titles[step]}</h2><span className="formHint">这些信息以后都可以修改。</span></div>
      ...
      <form onSubmit={submit} className="formStack">
        {step === 0 && <>
          ...
          <fieldset><legend>你现在主要想做什么？（可多选）</legend><div className="choiceGrid">{goalOptions.map((goal) => <label key={goal}><input aria-label={goal} type="checkbox" checked={selectedGoals.includes(goal)} onChange={() => toggleGoal(goal)} />{goal}</label>)}</div>{goalError && <p className="formError" role="alert">{goalError}</p>}</fieldset>
        </>}

        {step === 1 && <>
          <label htmlFor="budget">每周大约愿意花多少餐费？<input id="budget" type="number" min="0" inputMode="decimal" required value={profile.weeklyFoodBudget} onChange={(event) => updateField("weeklyFoodBudget", event.currentTarget.value)} placeholder="例如 350" /><span className="formHint">如果还没录入真实价格，这个预算只会按参考估价计算。</span></label>
          ...
          <fieldset><legend>你常遇到哪些吃饭场景？</legend><div className="choiceGrid">{sceneOptions.map((scene) => <label key={scene}><input aria-label={scene} type="checkbox" checked={profile.mealScenes.includes(scene)} onChange={() => toggleOption("mealScenes", scene)} />{scene}</label>)}</div></fieldset>
        </>}
        ...
        {step === 2 && <>
          ...
          <fieldset><legend>家里方便使用的做饭方式</legend><div className="choiceGrid">{kitchenOptions.map((option) => <label key={option}><input aria-label={option} type="checkbox" checked={profile.kitchenCapabilities.includes(option)} onChange={() => toggleOption("kitchenCapabilities", option)} />{option}</label>)}</div></fieldset>
          ...
          <p className="formHint">完成建档后，可在“我的 → 我的食材”补充常买食物和常见外食的实际价格，预算会优先使用这些数据。</p>
        </>}

        <div className="formActions">{step > 0 && <button type="button" className="secondaryButton" onClick={() => setStep((current) => current - 1)}>上一步</button>}<button className="primaryButton" type="submit">{step === 2 ? "生成我的第一周方案" : "下一步"}</button></div>
      </form>
    </main>
  );
}
