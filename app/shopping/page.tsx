...
import { getLocalWeekStartKey, parseLocalDateKey } from "@/lib/local-calendar";
...
export default function ShoppingPage() {
  ...
  const [weekStart, setWeekStart] = useState(() => getLocalWeekStartKey());
  ...
  useEffect(() => {
    ...
    if (requestedWeek && parseLocalDateKey(requestedWeek)) setWeekStart(requestedWeek);
  }, []);
  useEffect(() => {
    ...
    const storedItems = loadShoppingList(weekStart);
    const nextItems = storedItems.length > 0 ? storedItems : deriveShoppingList(plan);
    setHasPlan(true);
    setItems(nextItems);
    setLoadedWeekStart(weekStart);
    if (storedItems.length === 0) saveShoppingList(weekStart, nextItems);
  }, [weekStart]);
  useEffect(() => {
    if (items && loadedWeekStart === weekStart) saveShoppingList(weekStart, items);
  }, [items, loadedWeekStart, weekStart]);
  ...
  return (
    <main className="appShell withNav">
      <header className="pageHeader centered"><p className="homeKicker">按状态查看</p><h1>{weekStart === getLocalWeekStartKey() ? "本周采购清单" : "下周采购清单"}</h1><p>买到后勾一下即可。</p></header>
